...
class P3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.input_channel = 3 if modality=='RGB' else 1  # 1 is for tra 
        self.conv1 = nn.Conv3d(self.input_channel,
                       64,
                       kernel_size=7,
                       stride=(1, 2, 2),
                       padding=(3, 3, 3),
        ...
        self.cnt=0
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool3d(kernel_size=(3, 3, 3), stride=2, padding=(1,1,1))       # pooling layer for conv1.
        # self.maxpool = nn.MaxPool3d(kernel_size=(2, 3, 3), stride=1, padding=(0,1,1))       # pooling layer for conv1.
        ...
        self.avgpool = nn.AvgPool2d(kernel_size=(last_size, last_size), stride=1)              # pooling layer for res5.
        ...
        self.input_size=(self.input_channel,32,256,256)       # input of the network
    ...
    def forward(self, x):
        ...
        if self.sample_duration==32:
            x = self.maxpool_2(x)
        # print('after(x)', x.shape)
        sizes=x.size()
        x = x.view(-1,sizes[1],sizes[3],sizes[4])  #  Part Res5
        # print('x.view-1', x.shape)    
        x = self.layer4(x)
        # print('layer4(x)', x.shape) 
        x = self.avgpool(x)    
        # print('avgpool(x)', x.shape) 
        x = x.view(-1,self.fc.in_features)
        # print('x.view-2', x.shape)
...
if __name__ == '__main__':
    ...
    model = P3D63(pretrained=False,modality='TRA',sample_duration=32,num_classes=2)
    model = model.cuda()
    # data=torch.autograd.Variable(torch.rand(8,1,32,256,256)).cuda()   # if modality=='Flow', please change the 2nd dimension 3==>2
    # out=model(data)
    # print (out.size(),out)
    video = torch.randn(1, 1, 32, 256, 256).cuda()
    flops, params = profile(model, (video,))
    gflops = flops/1000000000
    mparams = params/1000000
    print(f"Model FLOPs: {flops} - Model parameters: {params}")
    print(f"Model FLOPs: {gflops:.1f}G - Model parameters: {mparams:.1f}M")
...
# 3. Based on A, match (spatial-temporal matching) using the ground truth and detection values.
...
#    Downsample uniformly (first take 32 frames) based on the devided frames, and save them.
...
rgb_features_output = 'rgb_volumes_region'   # RGB feature folder
if not os.path.exists(rgb_features_output):
    os.makedirs(rgb_features_output)
    
tra_features_output = 'tra_volumes_region'   # Trajectory feature folder
...
for file_name in tqdm(video_num, desc='Processing files'):    
    ...
    for group_id, label_boxes in label_dict.items():
        ...
        if os.path.exists(rgb_save_path) or os.path.exists(tra_save_path):
            # print("File already exists, skipping...")
            continue
            
        crop_images(video_path, rgb_save_path, crop_rectangle, start_frame-1, end_frame-1, frame=32)
        crop_images(trajectory_path, tra_save_path, crop_rectangle, start_frame-1, end_frame-1, frame=32)
    ...
    for box_id, det_boxes in top_det_dict.items():
        if box_id in att_top_ids:   # Skip matched ids
            ...
            crop_images(trajectory_path, tra_save_path, crop_rectangle, det_start-1, det_end-1, frame=32)
